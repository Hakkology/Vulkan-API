...
MeshManager::~MeshManager() {
    // Destroy all meshes on destruction
    for (auto& pair : meshes) {
        destroyMesh(pair.second.get());
    }
}

Mesh* MeshManager::createMesh(const std::vector<Vertex>& vertices) {
    // Create a new mesh object
    std::vector<Vertex>* nonConstVertices = const_cast<std::vector<Vertex>*>(&vertices);
...
const std::unordered_map<int, std::unique_ptr<Mesh>>& MeshManager::getAllMeshes() const {
    return meshes;
}