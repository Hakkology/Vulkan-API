...
bool DeviceManager::pickPhysicalDevice(VkInstance instance,
...
VkDevice DeviceManager::getLogicalDevice() const { return logicalDevice; }

bool DeviceManager::createLogicalDevice(VkSurfaceKHR surface) {
  // Find queue families that are required for the desired device
  // functionalities
  ...
  // Collect unique queue family indices that are valid for this physical device
  // and surface
  ...
  for (int queueFamily : uniqueQueueFamilies) {
    ...
    queueCreateInfo.queueFamilyIndex =
        queueFamily; // Index of the family to create a queue from
    queueCreateInfo.queueCount = 1; // Number of queues to create
    queueCreateInfo.pQueuePriorities =
        &queuePriority; // Priority for the queues
    queueCreateInfos.push_back(
        queueCreateInfo); // Collect all queue creation information
  }
...
bool DeviceManager::checkDeviceSuitable(VkPhysicalDevice device,
  ...
  QueueFamilyIndices indices =
      VulkanUtils::findQueueFamiliesForSurface(device, surface);
  ...
  SwapChainDetails swapChainDetails =
      swapChainManager.getSwapChainDetails(device, surface);

  // Check swap chain adequacy
  bool swapChainAdequate = !swapChainDetails.formats.empty() &&
