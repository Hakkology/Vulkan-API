...
void Mesh::createVertexBuffer(std::vector<Vertex> *vertices) {
  ...
  bufferInfo.size =
      sizeof(Vertex) *
      vertices
          ->size(); // size of buffer (size of 1 vertex * number of vertices)
  bufferInfo.usage =
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; // multiple types of buffer possible,
  ...
  memoryAllocInfo.allocationSize =
      memRequirements.size; // index of memory type on physical device that has
                            // required bit flags.
  ...
  // into buffer after mapping.

  // Allocate memory to VkDeviceMemory
  result =
      vkAllocateMemory(device, &memoryAllocInfo, nullptr, &vertexBufferMemory);
  if (result != VK_SUCCESS) {
  ...
  memcpy(data, vertices->data(),
         (size_t)bufferInfo
             .size); // 3. Copy memory from vertices vector to the point.
  vkUnmapMemory(device, vertexBufferMemory); // 4. Unmap vertex buffer memory.
}

uint32_t Mesh::findMemoryTypeIndex(uint32_t allowedTypes,
  ...
  for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; i++) {
    if ((allowedTypes & (1 << i)) // Index of memory type must match
                                  // corresponding bit in allowed Types
        &&
        (memoryProperties.memoryTypes[i].propertyFlags & properties) ==
            properties) // desired property bit flags are part of memory flags.
    {
      return i;
    }
  }
