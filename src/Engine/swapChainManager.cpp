...
SwapChainDetails
SwapChainManager::getSwapChainDetails(VkPhysicalDevice physicalDevice,
  ...
  VkResult result = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      physicalDevice, surface, &swapChainDetails.surfaceCapabilities);
  ...
  if (formatCount != 0) {
    ...
    result = vkGetPhysicalDeviceSurfaceFormatsKHR(
        physicalDevice, surface, &formatCount, swapChainDetails.formats.data());
    if (result != VK_SUCCESS) {
...
bool SwapChainManager::createSwapChain(VkPhysicalDevice physicalDevice,
                                       VkDevice logicalDevice,
                                       VkSurfaceKHR surface, GLFWwindow *window,
                                       const SwapChainDetails &initialDetails) {
  ...
  if (physicalDevice == VK_NULL_HANDLE || logicalDevice == VK_NULL_HANDLE ||
      surface == VK_NULL_HANDLE || window == nullptr) {
  ...
  // std::cout << "Surface capabilities - minImageCount: " <<
  // swapchainDetails.surfaceCapabilities.minImageCount << ", maxImageCount: "
  // << swapchainDetails.surfaceCapabilities.maxImageCount << std::endl;

  // Output chosen surface format and present mode for debugging.
  // std::cout << "Chosen surface format: " <<
  // swapchainDetails.formats[0].format << std::endl; std::cout << "Chosen
  // present mode: " << swapchainDetails.presentationModes[0] << std::endl;
  ...
  // Calculate the number of images in the swap chain. Aim for 1 more than the
  // minimum to allow triple buffering. std::cout << "Calculating image count
  // for swap chain..." << std::endl;
  ...
  if (!isSwapChainAdequate(physicalDevice, surface)) {
    std::cerr << "Swap chain configuration is not supported by the device."
              << std::endl;
    return false;
  }
  ...
  swapChainCreateInfo.imageColorSpace =
      surfaceFormat.colorSpace;                   // Swapchain color space
  ...
  swapChainCreateInfo.imageArrayLayers =
      1; // amount of layers for each image in chain
  ...
  swapChainCreateInfo.compositeAlpha =
      VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR; // Draw opaque, how to handle blending
                                         // images with external graphics.
  swapChainCreateInfo.clipped =
      VK_TRUE; // Whether to clip parts of image not in view (behind the window,
               // off screen)
  ...
  swapChainCreateInfo.oldSwapchain =
      VK_NULL_HANDLE; // Link to old swapchain if any to quickly hand over
                      // responsibilities.

  if (imageCount < swapchainDetails.surfaceCapabilities.minImageCount ||
      (swapchainDetails.surfaceCapabilities.maxImageCount > 0 &&
       imageCount > swapchainDetails.surfaceCapabilities.maxImageCount)) {
    std::cerr << "Image count " << imageCount << " is out of the allowed range."
              << std::endl;
    return false;
  }

  if (!swapchainDetails.formats.size() ||
      !swapchainDetails.presentationModes.size()) {
    std::cerr
        << "Required swap chain formats or presentation modes not available."
        << std::endl;
    return false;
  }

  // Print the calculated image count for debugging
  // std::cout << "Final image count for swap chain: " << imageCount <<
  // std::endl;
  ...
  VkResult result = vkCreateSwapchainKHR(logicalDevice, &swapChainCreateInfo,
                                         nullptr, &swapchain);
  if (result != VK_SUCCESS) {
    std::cerr << "Failed to create a swap chain! Error code: " << result
              << std::endl;
  ...
  for (VkImage image : images) {
    ...
    swapchainImage.imageView = createImageView(
        logicalDevice, image, m_chosenFormat, VK_IMAGE_ASPECT_COLOR_BIT);
...
VkImageView SwapChainManager::createImageView(VkDevice logicalDevice,
  ...
  viewCreateInfo.components.r =
      VK_COMPONENT_SWIZZLE_IDENTITY; // Swizzle effect to change pixel rbg
                                     // value.
  viewCreateInfo.components.g =
  ...
  viewCreateInfo.subresourceRange.aspectMask =
      aspectFlags; // which aspect of image to view
...
VkPresentModeKHR SwapChainManager::chooseBestPresentationMode(
...
bool SwapChainManager::isSwapChainAdequate(VkPhysicalDevice physicalDevice,
                                           VkSurfaceKHR surface) {
  // Check surface capabilities
  VkSurfaceCapabilitiesKHR capabilities;
  VkResult capResult = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      physicalDevice, surface, &capabilities);
  if (capResult != VK_SUCCESS) {
    std::cerr << "Failed to get surface capabilities, error code: " << capResult
              << std::endl;
    return false;
  }
  ...
  if (formatResult != VK_SUCCESS || formats.empty()) {
    std::cerr
        << "Failed to get surface formats or no formats available, error code: "
  ...
  if (presentModeResult != VK_SUCCESS || presentModes.empty()) {
    std::cerr << "Failed to get presentation modes or no modes available, "
                 "error code: "
              << presentModeResult << std::endl;
    return false;
  }

  // Log the counts for debugging

  // Ensure that the formats and present modes are not empty and there is a
  // sufficient image count
  bool isAdequate =
      !formats.empty() && !presentModes.empty() &&
      (capabilities.maxImageCount > 0 || capabilities.maxImageCount == 0);
  if (!isAdequate) {
    std::cerr << "Swap chain is not adequate based on the available data."
              << std::endl;
  }
