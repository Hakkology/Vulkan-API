...
FrameManager::FrameManager(VkDevice device, SwapChainManager &swapchainManager,
...
void FrameManager::createFrameBuffers(
  ...
  if (depthImageViews.size() != swapchainImages.size()) {
    throw std::runtime_error(
        "Depth image views count does not match swapchain images count!");
  }
  ...
  for (size_t i = 0; i < swapchainFrameBuffers.size(); i++) {
    ...
    std::array<VkImageView, 2> attachments = {
        swapchainImages[i].imageView, depthImageViews[i]
        // these need to be ordered in the same way as colour attachments in
        // render pass.
    };
    ...
    framebufferCreateInfo.width =
        swapchainManager.getChosenExtent().width; // framebuffer width.
    framebufferCreateInfo.height =
        swapchainManager.getChosenExtent().height; // framebuffer height.
...
void FrameManager::cleanup() {
  ...
  for (auto &framebuffer : swapchainFrameBuffers) {
    if (framebuffer != VK_NULL_HANDLE) {
      ...
      framebuffer =
          VK_NULL_HANDLE; // Explicitly set to VK_NULL_HANDLE after destruction
    }
  }
